...
# .. cmake:command:: swift_application
#
#   Compiles Swift application sources and integrates them with Zephyr's app target.
#
#   This function handles the complete build process for Swift applications:
#   - Source file discovery in src/ directory
#   - Swift library dependency resolution
#   - Cross-compilation for embedded targets
#   - Integration with Zephyr's build system
#   - Final binary post-processing
#
function(swift_application)
  # Enable Swift language support and configure compiler settings
  _enable_swift()

  # Discover all Swift source files in the src/ directory
  # This follows Zephyr's convention where application sources go in src/
  file(GLOB_RECURSE SWIFT_SOURCES "${CMAKE_CURRENT_SOURCE_DIR}/src/*.swift")

  # Include the core Zephyr Swift application bootstrap code
  # This provides the main() function and entrypoint() delegation
  set(SWIFT_APPLICATION "${SWIFT_MODULE_DIR}/zephyr/src/Application.swift")

  # Define output file paths for the compiled Swift application
  # Object file: Contains the compiled machine code for the application
  ...
  # Discover available Swift libraries to link against
  # TODO: Make this discovery dynamic by scanning for *_compile targets
  set(AVAILABLE_LIBS "")
  ...
  # Build the dependency chain to ensure proper compilation order
  # Start with the application's Swift source files
  set(COMPILE_DEPS ${SWIFT_SOURCES})

  # Ensure the core Zephyr Swift library is compiled first
  # This is essential since applications typically import Zephyr functionality
  if(TARGET Zephyr_compile)
    list(APPEND COMPILE_DEPS Zephyr_compile)
  endif()

  # Add dependencies on all discovered Swift libraries
  # This ensures they are compiled before the application
  ...
  # Integrate the compiled Swift object file with Zephyr's app target
  # This adds the Swift code to the final executable
  ...
  # Link all discovered Swift libraries
  # This ensures Swift libraries are available at runtime
  foreach(LIB ${AVAILABLE_LIBS})
    if(TARGET ${LIB})
  ...
  # Set the linker language to C to ensure proper integration with Zephyr
  # This is necessary because the final executable links C and Swift code together
  set_target_properties(app PROPERTIES LINKER_LANGUAGE C)

  # Post-build step: Clean up Swift-specific sections from the final binary
  # The .swift_modhash section is used during compilation but not needed in the final embedded binary
  # Removing it reduces binary size and eliminates unnecessary embedded metadata
  add_custom_command(
    TARGET app
    POST_BUILD
